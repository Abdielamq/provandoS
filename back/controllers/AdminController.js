...
const crear_cliente = async function(req,res){
    //
    var data = req.body;
    var clientes_arr = [];

    clientes_arr = await Cliente.find({dni:data.dni});
    console.log(data);
    if(clientes_arr.length == 0){
        if(!data.password){
            data.password = 'undefinet';
            data.email = 'undefinet';
...
const enviar_email_pedido_compra = async function(venta){
    try {
        ...
        var transporter = nodemailer.createTransport(smtpTransport({
            ...
            host: 'smtp.gmail.com',
            auth: {
                user: 'balazeroshanchezf@gmail.com',
                pass: 'ahkexuyuqgribhmj'
            }
        }));
        ...
        readHTMLFile(process.cwd() + '/mails/email_pedido.html', (err, html)=>{
            ...
            var mailOptions = {
                from: 'balazeroshanchezf@gmail.com',
                to: orden.cliente.email,
                subject: 'Gracias por tu orden, NIkitaSport.',
                html: htmlToSend
            };
