...
export class CreateVentasComponent implements OnInit {
      ...
      response=>{
          ...
          response.data.forEach((element:any) => {
              this.variedades.push({
                ...
                color: element.producto.color,
                genero: element.producto.genero,
              })
          });     
  ...
  registrar_venta(){
    ...
    this.venta.subtotal = this.neto_pagar;
    this.venta.total_pagar = this.neto_pagar - this.descuento;
  ...
  aplicarDescuento(){
    if(this.neto_pagar >= 0){
      if(this.descuento <= this.neto_pagar){
        this.total_pagar = this.neto_pagar - this.descuento;
      }else{
        iziToast.show({
            title: 'ERROR',
            titleColor: '#FF0000',
